...
public class IntegratedPaymentController implements Controller {
	...
	public String handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String[] seatNo = request.getParameter("seatNo").trim().split("\\s+");
		...
		return "/reservation/integrated/integratedPayment.jsp";
	}
	
	
	private ScheduleVO requestApi(int db,int no) throws IOException{
		URL url = new URL("http://172.31.7.184:8081/Cinema-API/paymentForm.do?db="+db+"&no="+no);
